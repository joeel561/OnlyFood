<template>
    <div class="recipe-listing-overview overview col-12">
        ...
        <div class="offcanvas offcanvas-start offcanvas-navigation" tabindex="-1" id="offcanvasFilter"
            aria-labelledby="offcanvasFilterLabel">
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close">
                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="36" height="36"
            ...
            <div class="offcanvas-body accordion" id="filterIngredientsOverview">
                ...
                <div class="filter-search">
                    ...
                    <div class="filter-box d-flex align-items-center" v-for="tag in filteredTags" :key="tag.id">
                        <input type="checkbox" :id="tag.id" class="form-check-input" :value="tag.id"
                            v-model="selectedTags" @click="filterTag(tag.id)" />
        ...
        <div class="row justify-content-center gx-2">
            <div class="no-recipes" v-if="recipes.length == 0">
                ...
                <div class="recipe-overview-filter-btn d-flex justify-content-end col-md-12">
                    <a class="btn btn-secondary" data-bs-toggle="offcanvas" href="#offcanvasFilter" role="button"
                        aria-controls="offcanvasFilter">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="icon icon-tabler icon-tabler-adjustments-horizontal d-block d-md-none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round"
                            stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <div v-if="this.$route.name == 'overview'">
                    <h2>Recipe Overview</h2>
                    <p>You don't have any recipes yet.</p>
                </div>
                <div v-else-if="this.$route.name == 'explore'">
            ...
            <div class="col-12 recipe-overview-panel" v-else>
                ...
                <div class="recipe-overview-search-results d-flex justify-content-center align-items-center flex-column"
                    v-if="resultsFound">
                    <p>Your search results:</p>
                    <div class="d-flex justify-content-center">
                        <div class="recipe-tag recipe-overview-tag" v-for="result in resultsFound">
                            {{ result }}
                        </div>
                    </div>
                </div>
                <div class="recipe-overview-container d-flex flex-wrap" ref="recipe">
                    <div class="col-6 col-md-4 col-lg-3 p-1 p-md-3" v-for="recipe in recipes" :key="recipe.id">
                        <router-link :to="{ name: 'recipeDetail', params: { id: recipe.id } }"
                            class="recipe-overview-item-link">
...
export default {
    ...
    computed: {
        filteredTags() {
            return this.tags.filter((tag) =>
                tag.name.toLowerCase().includes(this.tagKeyword.toLowerCase())
            );
        },
    ...
    methods: {
        loadData() {
            this.$axios
                .get("/api/recipes/" + this.$route.name)
                .then((response) => {
