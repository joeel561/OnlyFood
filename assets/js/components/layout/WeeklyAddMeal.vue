<template>
    <div class="add-more-meals-box">
        <button class=" btn btn-primary weekly-plan-box--btn" data-bs-toggle="modal" data-bs-target="#weeklyAddMeal">
        ...
        <div class="modal fade weekly-plan-modal" id="weeklyAddMeal"
            tabindex="-1" 
            aria-labelledby="weeklyAddMealLabel"
            aria-hidden="true">
...
<script>
    export default {
        ...
        async mounted() {
            try {
                let response = await this.$axios.get('/api/weekly-plan/showRecipes');
