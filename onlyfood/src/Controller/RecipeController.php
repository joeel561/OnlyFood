...
class RecipeController extends AbstractController
{
    ...
    public function createRecipe(Request $request, SerializerInterface $serializer): Response
    {
        $user = $this->getUser();
        $content = json_decode((string) $request->get('recipe'), true, 512, JSON_THROW_ON_ERROR);
        ...
        foreach ($content['tags'] as $tag) {
             ...
             if (in_array($tag['name'], $inUseTags)) {
                 if (!$tagRepo instanceof \App\Entity\Tag) {
                     $tagRepo = new Tag();
        ...
        foreach ($content['ingredients'] as $ingredient) {
            ...
            if (!$ingredientEntity instanceof Ingredients) {
    ...
    public function show(Request $request, SerializerInterface $serializer): Response
    {
        ...
        $newResponse = ['recipe' => $recipeJson, 'weeklyPlans' => $weeklyPlanJson, 'isUserRecipe' => $isUserRecipe, 'isUserLoggedIn' => $userId];
    ...
    public function likeRecipe(Request $request, SerializerInterface $serializer): Response
    {
        $user = $this->getUser();
     ...
     * @throws Exception
     */
    private function setIngredients(Ingredients $ingredientEntity, array $ingredient): void
    {
        if ($ingredient['name']) { 
