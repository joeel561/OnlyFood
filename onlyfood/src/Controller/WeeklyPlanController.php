...
class WeeklyPlanController extends AbstractController
{
    ...
    public function createWeeklyPlan(Request $request, SerializerInterface $serializer): Response
    {
        $user = $this->getUser();
    ...
    private function setWeeklyPlanContent($weeklyPlan, $content, $recipe): Response
    {
        ...
        return $this->json(['message' => 'Weekly plan created!'], Response::HTTP_OK);
    }

    #[Route(path: '/api/weekly-plan/remove/', name: 'app_remove_weekly_plan')]
    public function removeWeeklyPlan(Request $request, SerializerInterface $serializer): Response
    {
        $user = $this->getUser();
        $content = json_decode($request->getContent(), true, 512, JSON_THROW_ON_ERROR);
    ...
    public function showRecipesToWeeklyPlan(Request $request, SerializerInterface $serializer): Response
    {
        $user = $this->getUser();
    ...
    public function todaysMealPlan(Request $request, SerializerInterface $serializer): Response
    {
        $user = $this->getUser();
        $content = json_decode($request->getContent(), true, 512, JSON_THROW_ON_ERROR);
        if ($content['date']) {
